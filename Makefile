...
.PHONY: p4-03-integration-check p4-04-check p4-04-integration-check p4-05-check p4-06-check p4-07-check p4-07-integration-check p4-07-postgres-integration-check p4-07-latency-integration-check p4-07-combined-integration-check
p4-03-check:
...
# Focused P4-04 persistence and ledger checks. The integration target requires Docker.
p4-04-check:
	mvn -f services/control-plane/pom.xml test
...
# Focused P4-06 deployment artifact checks. Docker/Helm runtime checks are
...
p4-07-postgres-integration-check:
	INFORSIGHT_RUN_P4_07_POSTGRES_INTEGRATION=1 mvn -f services/control-plane/pom.xml -Dtest=P407PostgresQualificationIntegrationTest test

p4-07-latency-integration-check:
	INFORSIGHT_RUN_P4_07_LATENCY_INTEGRATION=1 mvn -f services/control-plane/pom.xml -Dtest=P407LatencyQualificationIntegrationTest test

p4-07-combined-integration-check:
	INFORSIGHT_RUN_P4_07_COMBINED_INTEGRATION=1 mvn -f services/control-plane/pom.xml -Dtest=P407CombinedTopologyIntegrationTest test

p4-07-local-latency-check:
	$(PYTHON) scripts/run_p4_07_local_latency.py
