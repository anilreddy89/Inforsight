...
### Conservation case brief schema

[`conservation-case-brief.schema.json`](conservation-case-brief.schema.json) defines version `1.0.0` of the structured decision-support brief generated by the bounded case intelligence assistant (Phase 3.05):

- **Authority Boundary**: Structurally enforces `status: PENDING_HUMAN_REVIEW` and `authorized_to_act: false`.
- **Core Sections**: Executive summary, calibrated risk assessment, chronological factual timeline up to cutoff, intervention recommendations with uplift quadrant and talking points, and explicit list of disqualified actions.
- **Grounding Audit**: Captures post-processing verification status (`PASSED_CLEAN`, `PASSED_WITH_REDACTION`, `FALLBACK_TO_TEMPLATE`), entity verification count, and any detected hallucination violations.
- **Mandatory Disclaimer**: Requires statutory advisory notice warning against automated outreach.
