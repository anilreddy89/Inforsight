...
- [x] **R2-16 - Run fresh replacement statistical acceptance:** Completed on 2026-09-04 through [issue #92](https://github.com/anilreddy89/Inforsight/issues/92) and [PR #93](https://github.com/anilreddy89/Inforsight/pull/93), merge commit `82e767f`. Executed 120 inventory units across reserved acceptance seeds `20271201..20271220` and 3 rolling-origin temporal folds under Protocol 3.0.0. Primary signal recovery passed (median AUC 0.7031 >= 0.68, 20/20 seed consistency, AP lift +0.1344, Brier skill +0.0658); 4 secondary rules failed thresholds (`CTRL-NULL-INTERVAL-COVERAGE`, `CTRL-SHUFFLE-INTERVAL-COVERAGE`, `ORACLE-CONDITIONAL-ORDERING`, `LEARNING-VARIANCE-CONTRACTION`), resulting in mechanical decision `redesign`. Clean-room holdout remains `not_materialized`. Resumed Phase 2 work (P2-08..P2-12) remains paused.
- [ ] **R2-16A - Generation v6 acceptance remediation and protocol 3.1.0 amendment:** Planned. Adopt ADR 0013 and approve Statistical Acceptance Protocol `3.1.0` to address finite-sample binomial coverage, quadrature discretization, and variance floor secondary heuristics while keeping all primary signal recovery gates strictly intact. Re-evaluate the 120 inventory units under Protocol `3.1.0`. Only merged `proceed` resumes governed Phase 2 work.
