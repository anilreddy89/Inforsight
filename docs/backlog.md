...
- [ ] **P2-08 - Probability calibration and operational thresholds (IN PROGRESS - Issue #96):** In progress on branch `feat/96-p2-08-probability-calibration` through [issue #96](https://github.com/anilreddy89/Inforsight/issues/96). Following the merged `proceed` decision of Phase 2R.16A (PR #95), fit probability calibration using the separately designated non-test calibration data (role `calibration`, 10% of cohort) and report discrimination, calibration, precision at operational review capacity, recall in high-risk bands, threshold tradeoffs, uncertainty, and explicit false-positive cost assumptions. Do not access the final release holdout for model or threshold selection.
