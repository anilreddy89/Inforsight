#!/usr/bin/env python3
"""Run or reproduce the governed R2-14D v6 development qualification."""
...
from inforsight_simulator.v6_qualification import (  # noqa: E402
    DEVELOPMENT_SEEDS, GATE_IDS, aggregate_qualification, build_readiness_manifest,
    execute_qualification_seed,
)

DESTINATIONS = {
    "manifest": ROOT / "docs/experiments/phase-02r-14d-v6-qualification-manifest.json",
    "report": ROOT / "docs/experiments/phase-02r-14d-v6-qualification-report.md",
    "decision": ROOT / "docs/experiments/phase-02r-14d-v6-qualification-decision.md",
}
...
def render(aggregate: dict) -> dict[str, bytes]:
    ...
    report = ["# Phase 2R.14D Generation v6 Development Qualification Report", "", "Issue: #88", "",
    ...
    if set(aggregate["gates"]) != set(GATE_IDS):
        raise ValueError("R2-14D report gate schema changed")
    report.extend(f"| `{gate}` | `{result['status']}` |"
                  for gate in GATE_IDS
                  for result in (aggregate["gates"][gate],))
    report.extend(["", "This is development qualification of the Generation v6 bounded sigmoid substrate.",
                   "Future acceptance and the final holdout remain `not_materialized`.", ""])
    decision = ["# Phase 2R.14D Generation v6 Qualification Decision", "",
    ...
    if aggregate["decision"] == "qualified":
        ...
        decision.append("R2-15 remains blocked; return to a reviewed design version.")
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", type=Path, default=ROOT / "tmp/r2-14d-qualification")
    ...
    if args.seed is not None:
        if readiness["readiness_decision"] != "authorized":
            print("R2-14D readiness failed closed", file=sys.stderr)
            return 1
    ...
    if args.write:
        ...
        for seed in DEVELOPMENT_SEEDS:
            ...
            if not path.is_file():
                print(f"Executing qualification for seed {seed}...")
                result = execute_qualification_seed(seed)
                path.write_text(json.dumps(result, allow_nan=False, indent=2,
                                           sort_keys=True) + "\n")
        ...
        for name, path in DESTINATIONS.items():
            ...
            print(f"Wrote {path.relative_to(ROOT)}")
        print(f"R2-14D qualification complete. Decision: {aggregate['decision']}")
        return 0
    if args.check:
        if any(not path.is_file() for path in paths) and DESTINATIONS["manifest"].is_file():
            aggregate = json.loads(DESTINATIONS["manifest"].read_text())
        elif any(not path.is_file() for path in paths):
            print("complete R2-14D seed evidence is missing", file=sys.stderr)
            return 1
        else:
            aggregate = aggregate_qualification(
                (json.loads(path.read_text()) for path in paths), readiness)
        ...
        if stale:
            print(f"R2-14D artifacts are stale: {', '.join(stale)}", file=sys.stderr)
            return 1
        print(f"R2-14D qualification artifacts reproduce. Decision: {aggregate['decision']}")
        return 0
    return 1
