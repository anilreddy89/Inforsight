...
public final class AuditLedgerVerifier {
    public Verification verify(List<AuditLedgerEntry> entries) {
        ...
        for (AuditLedgerEntry entry : entries) {
            // BIGSERIAL values are not transactional: a rolled-back insert can
            // leave an unused value without a missing committed ledger row.
            if (entry.sequence() <= previousSequence) {
                return Verification.invalid("NON_MONOTONIC_SEQUENCE", entry.sequence());
            }
