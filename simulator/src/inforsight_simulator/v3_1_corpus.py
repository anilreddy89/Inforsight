"""Event-first v3.1 corpus with the versioned arrears-driver remediation."""
...
from .v3_1_config import (
    ...
    V3_ROLE_COUNTS_PER_600, V3_ROLES, V31_SIMULATOR_CONTRACT_VERSION,
    V31CorpusConfig as V3CorpusConfig, artifact_id,
...
def competing_hazards(features: V3Features, frailty: float, month: int, *,
    ...
    # Contract 3.1.0 recalibrates both cause-specific intercepts by -0.70. The
    # shared shift preserves their relative odds while restoring the frozen
    # total-terminal-hazard bound after nonzero arrears is introduced.
    eta_lapse = -4.05 + _MONTH_OFFSETS[month - 1] + frailty + signal_scale * lapse_score + drift
    eta_surrender = -4.75 + _MONTH_OFFSETS[month - 1] + 0.50 * frailty + signal_scale * surrender_score + drift
...
def _behavior_events(config: V3CorpusConfig, policy_id: str, cutoff: datetime,
    ...
    arrears_uniform = primitive_uniform(
        config, "behavior_value", policy_id, "payment", episode, "arrears_days",
    )
    arrears = 1 + math.floor(60 * arrears_uniform) if failed else 0
