...
V3_SPLIT_VERSION = "3.2.0"
V3_CANDIDATE_SELECTION_MEMBERSHIP_VERSION = "3.2.0"
V3_EVALUATION_ACCEPTANCE_PROTOCOL_VERSION = "2.2.0"
...
V3_DIAGNOSTIC_VERSION = V3_CONTRACT_VERSION
V3_CANDIDATE_VERSION = V3_CONTRACT_VERSION
V3_FINAL_HOLDOUT_STATUS = "not_materialized"
V3_STRUCTURAL_SUPPORT_VERSION = "1.1.0"
...
def fit_preprocessor(fold: V3TemporalFold) -> V3Preprocessor:
    ...
    if len(artifact_ids) != 1:
        raise ValueError("preprocessing requires one v3 artifact identity")
    numeric = []
    ...
    return V3Preprocessor(
        V3_FEATURE_PIPELINE_VERSION, fold.name, next(iter(artifact_ids)),
...
def transform(
    fitted: V3Preprocessor, rows: Iterable[V3Observation], *, purpose: str, role: str,
) -> V3Matrix:
    ...
    for row in materialized:
        ...
        if not all(math.isfinite(value) for value in output):
            raise ValueError("v3 matrix contains a non-finite value")
...
def authorize(
    train: V3Matrix, matrix: V3Matrix, fitted: V3Preprocessor, *, model_sha256: str,
) -> V3ScoringAuthorization:
    """Bind scoring permission to complete v3 content and provenance."""

    if train.purpose != "fit" or matrix.purpose not in {"fit", "selection", "acceptance"}:
        raise ValueError("unsupported v3 scoring purpose")
    ...
    fields = (
        ...
        V3_EVALUATION_ACCEPTANCE_PROTOCOL_VERSION, matrix.purpose, matrix.fold,
    ...
    return V3ScoringAuthorization(*fields, _digest(fields))


def validate_authorization(
    authorization: V3ScoringAuthorization, matrix: V3Matrix, *,
    ...
    if authorization.version != V3_SCORING_AUTHORIZATION_VERSION:
        raise ValueError("v3 scoring authorization version mismatch")
    if authorization.split_version != V3_SPLIT_VERSION:
        raise ValueError("v3 scoring authorization split version mismatch")
    if authorization.protocol_version != V3_EVALUATION_ACCEPTANCE_PROTOCOL_VERSION:
        raise ValueError("v3 scoring authorization protocol version mismatch")
    if authorization.authorization_sha256 != _digest(fields):
        raise ValueError("v3 scoring authorization integrity failure")
    metadata = (
        authorization.purpose, authorization.fold, authorization.role,
        authorization.artifact_id, authorization.observation_ids,
    ...
    if metadata != expected:
        raise ValueError("v3 scoring authorization metadata mismatch")
    if authorization.matrix_sha256 != matrix_digest(matrix):
        raise ValueError("v3 scoring authorization matrix mismatch")
    if authorization.target_sha256 != target_digest(matrix.targets):
        raise ValueError("v3 scoring authorization target mismatch")
    if authorization.fit_matrix_sha256 != fit_matrix_sha256:
        raise ValueError("v3 scoring authorization fit mismatch")
    if authorization.preprocessor_sha256 != preprocessor_sha256:
        raise ValueError("v3 scoring authorization preprocessing mismatch")
    if authorization.model_sha256 != model_sha256:
        raise ValueError("v3 scoring authorization model mismatch")


def compare_candidates(
    train: V3Matrix, selection: V3Matrix, fitted: V3Preprocessor,
) -> dict[str, Any]:
    ...
    return {
        "versions": {
            "candidate": V3_CANDIDATE_VERSION,
            "selection_membership": V3_CANDIDATE_SELECTION_MEMBERSHIP_VERSION,
...
def diagnostics(train: V3Matrix, selection: V3Matrix, fitted: V3Preprocessor) -> dict[str, Any]:
    ...
    for flag in flags:
        ...
        if flag["kind"] == "fit_constant" and group != "missingness":
            ...
            decision = "allow"
            rationale = (
                "approved public v3 feature; full multi-seed control evidence remains R2-11 work"
            )
        dispositions.append({
    ...
    return {
        "diagnostic_version": V3_DIAGNOSTIC_VERSION,
...
def preprocessor_digest(fitted: V3Preprocessor) -> str:
...
def _feature_map(row: V3Observation) -> dict[str, Any]:
    if row.observation_contract_version != V3_CONTRACT_VERSION:
        raise ValueError("feature extraction requires observation contract 3.0.0")
    # Validate structurally across versioned simulator modules that retain the
    # same frozen 17-field public feature surface.
    validate_v3_feature_payload(asdict(row.features))
    if set(row.feature_lineage) != set(V3Features.__dataclass_fields__):
        raise ValueError("feature lineage must cover every public v3 feature")
...
def _candidate_result(
    matrix: V3Matrix, probabilities: tuple[float, ...], state: dict[str, Any],
) -> dict[str, Any]:
