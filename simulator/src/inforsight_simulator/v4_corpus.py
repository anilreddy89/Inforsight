"""Event-first v4 corpus implementing the frozen R2-14 substrate."""
...
from .v4_config import (
    V4_COEFFICIENT_REGISTRY_VERSION, V4_SIMULATOR_CONTRACT_VERSION,
...
V4_EVENT_SCHEMA_VERSION = "4.0.0"
V4_OBSERVATION_SCHEMA_VERSION = "4.0.0"
V4_ORACLE_SIDECAR_VERSION = "4.0.0"
V4_QUADRATURE_VERSION = "gauss-hermite-32-sd-0.20-v1"
V4_ARTIFACT_VERSION = "1.0.0"
...
class V4Features(V3Features):
    """The public 17-feature surface is preserved under a separate v4 type."""


@dataclass(frozen=True)
class V4Observation(V3Observation):
    features: V4Features


@dataclass(frozen=True)
class V4OracleRecord(V3OracleRecord):
    """Protected v4 oracle sidecar record."""


@dataclass(frozen=True)
class V4Corpus:
    histories: tuple[tuple[dict[str, Any], ...], ...]
    observations: tuple[V4Observation, ...]
    oracle_sidecar: tuple[V4OracleRecord, ...]
    provenance: dict[str, Any]


def reconstruct_v4_features(events: Iterable[dict[str, Any]], cutoff: datetime
                            ) -> tuple[V4Features, dict[str, tuple[str, ...] | str]]:
    features, lineage = reconstruct_v3_features(events, cutoff)
    result = V4Features(**asdict(features))
    validate_feature_lineage(result, lineage, visible_events(events, cutoff))
    return result, lineage


def public_mechanism_terms(features: V4Features) -> dict[str, float]:
    """Return exact clipped public cutoff terms used by both v4 hazards."""
...
def competing_hazards(features: V4Features, frailty: float, month: int, *,
    ...
    lapse = {
        "tenure": -0.16, "premium": 0.24, "quarterly": 0.12,
        "semiannual": 0.20, "annual": 0.28, "recent_delay": 0.84,
        "failed_payments": 1.40, "retries": 0.36, "recoveries": -0.60,
        "arrears": 1.10, "on_time_rate": -0.90, "rolling_payments": -0.20,
        "notices": 0.48, "contacts": 0.24, "failed_x_arrears": 0.44,
        "payment_missing": 0.0, "contact_missing": 0.0,
    }
    surrender = {
        "tenure": 0.08, "premium": 0.36, "quarterly": 0.08,
        "semiannual": 0.12, "annual": 0.16, "recent_delay": 0.24,
        "failed_payments": 0.40, "retries": 0.10, "recoveries": -0.16,
        "arrears": 0.30, "on_time_rate": -0.24, "rolling_payments": -0.08,
        "notices": 0.36, "contacts": 0.44, "failed_x_arrears": 0.16,
        "payment_missing": 0.0, "contact_missing": 0.0,
    }
    lapse_score = sum(lapse[name] * z[name] for name in z)
    surrender_score = sum(surrender[name] * z[name] for name in z)
    eta_lapse = -4.85 + _MONTH_OFFSETS[month - 1] + frailty + signal_scale * lapse_score + drift
    eta_surrender = -5.55 + _MONTH_OFFSETS[month - 1] + 0.50 * frailty + signal_scale * surrender_score + drift
    lapse_exp, surrender_exp = math.exp(eta_lapse), math.exp(eta_surrender)
    denominator = 1 + lapse_exp + surrender_exp
    result = lapse_exp / denominator, surrender_exp / denominator, 1 / denominator
    if enforce_generated_bound and result[0] + result[1] >= 0.20:
        raise ValueError("total terminal hazard must remain below 0.20")
    return result


def cumulative_incidence(features: V4Features, frailty: float, *, signal_scale: float,
...
def observable_oracle(features: V4Features, *, signal_scale: float,
                      drift: float) -> tuple[float, float, float]:
    nodes, weights = _QUADRATURE_NODES, _QUADRATURE_WEIGHTS
    z = public_mechanism_terms(features)
    lapse_coefficients = (-0.16, 0.24, 0.12, 0.20, 0.28, 0.84, 1.40, 0.36,
                          -0.60, 1.10, -0.90, -0.20, 0.48, 0.24, 0.44, 0.0, 0.0)
    surrender_coefficients = (0.08, 0.36, 0.08, 0.12, 0.16, 0.24, 0.40, 0.10,
                              -0.16, 0.30, -0.24, -0.08, 0.36, 0.44, 0.16, 0.0, 0.0)
    values = np.fromiter(z.values(), dtype=float)
    lapse_score = float(np.dot(lapse_coefficients, values))
    surrender_score = float(np.dot(surrender_coefficients, values))
    ...
    for offset in _MONTH_OFFSETS:
        lapse_exp = np.exp(-4.85 + offset + frailty + signal_scale * lapse_score + drift)
        surrender_exp = np.exp(-5.55 + offset + 0.50 * frailty
                               + signal_scale * surrender_score + drift)
        denominator = 1 + lapse_exp + surrender_exp
        lapse_total += survival * lapse_exp / denominator
        surrender_total += survival * surrender_exp / denominator
...
def generate_v4_corpus(config: V4CorpusConfig, *, enforce_hazard_bound: bool = True) -> V4Corpus:
    histories: list[tuple[dict[str, Any], ...]] = []
    observations: list[V4Observation] = []
    sidecars: list[V4OracleRecord] = []
    ...
    for cohort_index in range(config.cohort_count):
        ...
        for ordinal in range(config.policies_per_cohort):
            ...
            while cutoff + timedelta(days=90) <= config.watermark and not terminal:
                ...
                features, lineage = reconstruct_v4_features(events, cutoff)
                ...
                observations.append(V4Observation(
                    observation_contract_version=V4_OBSERVATION_SCHEMA_VERSION,
                    label_policy_version=V4_SIMULATOR_CONTRACT_VERSION,
                ...
                sidecars.append(V4OracleRecord(
                    sidecar_version=V4_ORACLE_SIDECAR_VERSION,
                    quadrature_version=V4_QUADRATURE_VERSION, artifact_id=aid,
    ...
    corpus = V4Corpus(tuple(histories), tuple(observations), tuple(sidecars), {
        "artifact_id": aid, "artifact_version": V4_ARTIFACT_VERSION,
        ...
        "simulator_contract_version": V4_SIMULATOR_CONTRACT_VERSION,
        "stream_set_id": stream_set_id(config),
    })
    validate_v4_corpus(corpus, config)
    return corpus


def validate_v4_corpus(corpus: V4Corpus, config: V4CorpusConfig) -> None:
    if len(corpus.histories) != config.policy_count:
        raise ValueError("v4 corpus has an incorrect policy count")
    if len(corpus.observations) != len(corpus.oracle_sidecar):
        raise ValueError("every v4 observation requires one oracle sidecar")
    if len({row.observation_id for row in corpus.observations}) != len(corpus.observations):
        raise ValueError("v4 observation identities must be unique")
...
def corpus_digest(corpus: V4Corpus) -> dict[str, str]:
    return {
...
def _scheduled_payments(config: V4CorpusConfig, policy_id: str, issued_at: datetime,
...
def _service_events(config: V4CorpusConfig, policy_id: str, cutoff: datetime,
...
def _event(config: V4CorpusConfig, policy_id: str, event_type: str,
    ...
    return {"schema_version": V4_EVENT_SCHEMA_VERSION, "event_id": event_id,
...
__all__ = [
    ...
    "V4Observation", "V4OracleRecord", "competing_hazards", "corpus_digest",
    "cumulative_incidence", "generate_v4_corpus", "observable_oracle",
