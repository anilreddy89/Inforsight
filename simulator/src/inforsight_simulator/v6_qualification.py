"""Fail-closed R2-14D Generation v6 development qualification and aggregate evidence."""
...
from .v6_config import V6CorpusConfig, stream_set_id
from .v6_corpus import (
    competing_hazards, generate_v6_corpus, public_mechanism_terms,
    reconstruct_v6_features,
)

R2_14D_ISSUE = 88
R2_14D_QUALIFICATION_VERSION = "1.0.0"
...
IMMUTABLE_INPUTS = (
    "docs/adr/0012-authorize-bounded-sigmoid-hazard-link-v6.md",
...
def evaluate_readiness(root: Path) -> tuple[ReadinessCheck, ...]:
    ...
    all_spent = set(SPENT_ACCEPTANCE_SEEDS) | set(SPENT_QUALIFICATION_SEEDS) | set(SPENT_DIAGNOSTIC_SEEDS)
    domains_disjoint = not (
        set(DEVELOPMENT_SEEDS) & set(FUTURE_ACCEPTANCE_SEEDS)
        or set(DEVELOPMENT_SEEDS) & all_spent
    )
    checks.append(ReadinessCheck(
        ...
        {"development": list(DEVELOPMENT_SEEDS), "future_status": "not_materialized",
         "spent_count": len(all_spent)}, "disjoint", evidence,
    ))
    ...
    tokens = {
        "docs/adr/0012-authorize-bounded-sigmoid-hazard-link-v6.md": (
            "issue #86", "20280201..20280220", "not_materialized"),
        "docs/modeling/phase-02r-14c-v6-bounded-sigmoid-substrate-contract.md": (
            "6.0.0", "3.0.0", "20280201..20280220", "0.10 + 0.05 = 0.1500 < 0.2000"),
    }
    missing_tokens = []
...
def build_readiness_manifest(root: Path) -> dict[str, Any]:
    ...
    return {
        "phase": "R2-14D", "issue": R2_14D_ISSUE,
        "qualification_version": R2_14D_QUALIFICATION_VERSION,
...
def execute_qualification_seed(seed: int) -> dict[str, Any]:
    if seed not in DEVELOPMENT_SEEDS:
        raise ValueError("seed is outside the R2-14D development domain")
    variants = {}
    for scenario, scenario_name in (("signal", "stable"), ("matched_null", "null_signal")):
        config = V6CorpusConfig(base_seed=seed, scenario=scenario_name)
        ...
        for fold in build_temporal_folds(compatible):
            ...
            mismatch = 0
            for row in fold.evaluation:
                rebuilt, _ = reconstruct_v6_features(
...
def aggregate_qualification(items: Iterable[dict[str, Any]], readiness: dict[str, Any]) -> dict[str, Any]:
    ...
    if tuple(row.get("seed") for row in seeds) != DEVELOPMENT_SEEDS:
        raise ValueError("complete ordered R2-14D seed evidence is required")
    if any(row.get("status") != "complete" or not row.get("matched_stream_set") for row in seeds):
        raise ValueError("invalid R2-14D seed evidence")
    ...
    max_hazard = max(variant["maximum_monthly_terminal_hazard"]
                     for row in seeds for variant in row["variants"].values())
    gates = {
        "observable_seed_recovery": sum(value >= 0.68 for value in oracle_auc) >= 16,
        "observable_aggregate_recovery": median(oracle_auc) >= 0.70,
        ...
        "hazard_validity": math.isfinite(max_hazard) and max_hazard < 0.20 and max_hazard <= 0.1500 + 1e-6,
    ...
    return {
        "phase": "R2-14D", "issue": R2_14D_ISSUE,
        "qualification_version": R2_14D_QUALIFICATION_VERSION,
        "substrate_contract_version": "6.0.0", "acceptance_protocol_version": "3.0.0",
        "coefficient_registry_version": "3.0.0", "random_stream_registry_version": "3.0.0",
        "readiness": readiness, "seed_count": len(seeds), "seeds": list(DEVELOPMENT_SEEDS),
        "summary": {"observable_oracle_auc_pass_count": sum(value >= 0.70 for value in oracle_auc),
...
__all__ = [
    "DEVELOPMENT_SEEDS", "FINAL_HOLDOUT_STATUS", "FUTURE_ACCEPTANCE_SEEDS",
    "GATE_IDS", "GOVERNED_FOLDS", "IMMUTABLE_INPUTS", "R2_14D_ISSUE",
    "R2_14D_QUALIFICATION_VERSION", "ReadinessCheck", "aggregate_qualification",
    "build_readiness_manifest", "canonical_sha256", "evaluate_readiness",
    "execute_qualification_seed", "planned_inventory", "readiness_decision",
]
