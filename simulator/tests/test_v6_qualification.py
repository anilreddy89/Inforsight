...
from inforsight_simulator.v6_qualification import (
...
class V6QualificationTests(unittest.TestCase):
    def test_domains_and_inventory_are_frozen(self) -> None:
        self.assertEqual(DEVELOPMENT_SEEDS, tuple(range(20280201, 20280221)))
    ...
    def test_committed_evidence_is_complete_and_mechanical(self) -> None:
        path = ROOT / "docs/experiments/phase-02r-14d-v6-qualification-manifest.json"
        if not path.is_file():
            self.skipTest("Manifest not yet generated (run python3 scripts/run_v6_qualification.py --write)")
        manifest = json.loads(path.read_text())
        self.assertEqual(manifest["issue"], 88)
        self.assertEqual(manifest["seed_count"], 20)
        self.assertEqual(len(manifest["seed_evidence"]), 20)
        self.assertEqual(manifest["decision"], "qualified")
        self.assertTrue(manifest["r2_15_authorized"])
        self.assertEqual(set(manifest["gates"]), set(GATE_IDS))
        self.assertGreaterEqual(manifest["summary"]["observable_oracle_auc_pass_count"], 16)
        self.assertLessEqual(manifest["summary"]["maximum_monthly_terminal_hazard"], 0.150001)
        self.assertLess(manifest["summary"]["maximum_monthly_terminal_hazard"], 0.20)
